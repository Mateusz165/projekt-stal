    ...
    fetch(`/api/admin/products/${id}`)
      ...
      .then((data) => {
        setForm({
          ...
          price: String(data.price), oldPrice: data.oldPrice ? String(data.oldPrice) : "",
          category: data.category, badge: data.badge || "",
          stock: data.stock, published: data.published,
          rating: String(data.rating), reviewCount: String(data.reviewCount),
        });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-black text-white" style={{ fontFamily: "var(--font-outfit)" }}>Edytuj produkt</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input required value={form.slug} onChange={(e) => update("slug", e.target.value)} className={inputCls} />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" min="0" step="0.01" value={form.price}
                onChange={(e) => update("price", e.target.value)} className={inputCls} />
            </div>
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.oldPrice}
                onChange={(e) => update("oldPrice", e.target.value)} className={inputCls} />
            </div>
            <div>
